...
// Pin/Unpin notice (moderator/admin only)
router.patch('/:id/pin', requireRole(['admin', 'moderator']), [
  ...
  try {
    ...
    res.json({ message: `Notice ${isPinned ? 'pinned' : 'unpinned'} successfully` });
  } catch (error) {
    console.error('Pin/unpin notice error:', error);
    res.status(500).json({ message: 'Server error' });
  }
});
