...
class SearchService {
  ...
  async searchUsers(query, options = {}) {
    ...
    const pipeline = [
      // Match users in the same neighbourhood
      {
        $match: {
          neighbourhoodId,
  ...
  async searchNotices(query, options = {}) {
    ...
    // Build search pipeline
    const pipeline = [
      // Match notices in the same neighbourhood
      {
        $match: {
          neighbourhoodId,
          status: 'active',
  ...
  async searchReports(query, options = {}) {
    ...
    // Build search pipeline
    const pipeline = [
      // Match reports in the same neighbourhood
      {
        $match: {
          neighbourhoodId,
          reportStatus: 'active'
  ...
  async searchChats(query, options = {}) {
    ...
    // Build search pipeline for chat groups
    const groupPipeline = [
      // Match chat groups in the same neighbourhood
      {
        $match: {
          neighbourhoodId,
          isActive: true,
  ...
  async searchMessages(query, options = {}) {
    ...
    // Build search pipeline
    const pipeline = [
      // Match messages in user's chats
      {
        $match: {
          $or: [
